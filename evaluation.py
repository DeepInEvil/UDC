...
def eval_model_v2(model, dataset, mode='valid', gpu=False, no_tqdm=False):
    ...
    for mb in data_iter:
        ...
        # Get scores
        scores_mb = F.sigmoid(model(context, response, cm, key_r, key_mask_r))
        scores_mb = scores_mb.cpu() if gpu else scores_mb
        scores.append(scores_mb.data.numpy())
...
def eval_model_hybrid_v1(model, dataset, mode='valid', gpu=False, no_tqdm=False):
    model.eval()
    scores = []
    ...
    for mb in data_iter:
        ...
        # Get scores
        scores_mb = F.sigmoid(model.predict(context, response))
        scores_mb = scores_mb.cpu() if gpu else scores_mb
        scores.append(scores_mb.data.numpy())

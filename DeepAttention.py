...
class GRUAttnmitKey(nn.Module):
    ...
    def forward_enc(self, x1, x2):
        ...
        x1_emb = self.emb_drop(self.word_embed(x1))
        x1_emb = torch.cat([x1_emb, key_emb_c], dim=-1)  # concat the embeddings
        #sprint (x1_emb[0])
        x2_emb = self.emb_drop(self.word_embed(x2))
        x1_emb = torch.cat([x1_emb, key_emb_r], dim=-1)
        ...
        sc, c = self.rnn(x1_emb)
        _, r = self.rnn(x2_emb)
        c = torch.cat([c[0], c[1]], dim=-1) # concat the hidden states on both direction.
        r = torch.cat([r[0], r[1]], dim=-1)

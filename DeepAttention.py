...
class GRUAttenmitIntent(nn.Module):

    def __init__(self, emb_dim, n_vocab, h_dim=256, pretrained_emb=None, pad_idx=0, gpu=False, emb_drop=0.7, max_seq_len=160):
        super(GRUAttenmitIntent, self).__init__()
        ...
        self.emb_drop = nn.Dropout(emb_drop)
        self.M = nn.Parameter(torch.FloatTensor(h_dim + 1, h_dim))
        self.b = nn.Parameter(torch.FloatTensor([0]))
        self.attn = nn.Linear(h_dim, h_dim)
        self.scale = 1. / math.sqrt(max_seq_len)
        self.out_hidden = nn.Linear(h_dim, 1)
    ...
    def getIntent(self, context):
        ...
        for i in range(context.size(0)):
            last_utr = context[i].split(63346)[-1].cpu().data.numpy()
            #print (last_utr, self.queries)
            for ids in last_utr:
                if ids in self.queries:
                    inte_vec[i] = 1

        return Variable(inte_vec.unsqueeze(1)).cuda()
